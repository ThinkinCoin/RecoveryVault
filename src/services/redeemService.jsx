...
// Depende de ethers v6 e do vaultService. Possui fallback local quando quoteRedeem não responde.
...
// Assinatura alternativa (sem `user`) – mantém a mesma ordem de retornos
const altIface = new ethers.Interface([
  "function quoteRedeem(address tokenIn,uint256 amountIn,address redeemIn,bytes32[] proof) view returns (bool,bool,uint256,uint256,uint256,uint256,uint256,uint8,uint8,uint256,uint8)"
]);
...
export async function safeQuoteRedeem(provider, vaultAddr, { user, tokenIn, amountIn, redeemIn, proof }) {
  ...
  try {
    ...
    if (rawA && rawA !== "0x") {
      ...
      console.debug("[redeemService] quoteRedeem matched signature A (with user)");
      return outA;
    }
  } catch {}

  // Assinatura B: sem `user`
  try {
    ...
    if (rawB && rawB !== "0x") {
      ...
      console.debug("[redeemService] quoteRedeem matched signature B (no user)");
      return outB;
    }
  } catch {}
...
/** USD inteiros (espelhando o contrato)
 * - wONE: oracleLatest (price,decimals), floor para USD inteiro
 * - USDC: face value
 * - Outros: fixedUsdPrice (1e18), floor
 */
export async function getUsdValue(provider, token, amountIn){
  ...
  const fixed = await vault.fixedUsdPrice(provider, t).catch(() => 0n); // 18d
  const dec = await getTokenDecimals(provider, t);
  if (fixed > 0n){
...
export async function getFeeSchedule(provider){
  const raw = await vault.getFeeTiers(provider).catch(() => ({ thresholds: [], bpsOut: [] }));
  const thresholds = Array.from(raw?.thresholds ?? []).map((x) => b(x));
  const bps = Array.from(raw?.bpsOut ?? raw?.bps ?? []).map((x) => n(x));
...
/** Cálculo local de quote (preview) – espelha o contrato */
...
/** Tokens suportados e saldos do usuário **/
...
/** MAX por limite diário (USD) e saldo **/
export async function computeMaxAmount(provider, user, token){
  ...
  if (toAddr(wone) === t){
    ...
    const fp = await vault.fixedUsdPrice(provider, t).catch(() => 0n); // 18d
    if (fp > 0n){
...
/** PREPARE: preview + plano (approve -> redeem), com fallback local quando quoteRedeem não responde **/
export async function prepareRedeem(provider, { user, tokenIn, amountHuman, redeemIn, proof = [] }){
  ...
  // redeemIn deve ser wONE ou USDC
  ...
  // Tenta quote on-chain (fonte da verdade)
  let q = null;
  try {
    ...
    q = {
      whitelisted: cleanProof.length > 0, // heurística p/ UI
  ...
  const display = {
    ...
    receiveText: String(q.refundAmount),
...
/** EXECUTE: approve (se necessário) -> redeem **/
export async function executeRedeem(signer, plan, opts = {}){
  ...
  // Sinaliza se houve RedeemProcessed (se o vaultService expõe tópicos)
  try{
